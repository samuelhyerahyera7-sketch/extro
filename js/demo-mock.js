// DEMO MODE ONLY — activated by adding ?demo=1&role=customer|restaurant|driver to a URL.
// Replaces the real Supabase client with an in-memory/localStorage-backed mock so the
// app can be clicked through end-to-end without a real backend. Not used in production
      ...
      if (this.type === 'select') {
        ...
        if (this.orderBy) {
          result.sort((a, b) => {
            ...
            return this.orderBy.asc ? (av > bv ? 1 : -1) : (av < bv ? 1 : -1);
          });
        }
      ...
      if (this.type === 'insert') {
        ...
        const inserted = items.map(item => {
          const row = {
            ...
            status: 'pending',
            payment_status: this.table === 'orders' ? 'paid' : undefined, // demo: skip real payment gateway
            is_open: true,
            available: true,
            driver_id: null,
            ...item,
  ...
  window.DEMO_SB = {
    ...
    functions: {
      async invoke(name) {
        ...
        return { data: {}, error: null }; // create-payfast-payment: no real gateway in demo
      },
    },
  };
})();
